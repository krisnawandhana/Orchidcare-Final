...
export const subscribeToDataTable = (callback) => {
    const dataTableSubscription  = supabase
      ...
      const realtimeDataSubscription = supabase
    ...
    return () => {
      supabase.removeChannel(dataTableSubscription ); // Unsubscribe ketika tidak dibutuhkan
      supabase.removeChannel(realtimeDataSubscription);
    };
  };
